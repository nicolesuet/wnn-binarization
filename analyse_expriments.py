...
def optimal_scatter_config(data, model_name):
    scatter_data = data[(data['model'] == model_name) & (data['encoding'] == 'Scatter Code')]
...
def best_encoding(data, model_name):
    ...
    return grouped['accuracy'].agg(['mean', 'std']).reset_index().sort_values(['dataset', 'mean'], ascending=[True, False])
...
def create_accuracy_graphs(dwn_data, wisard_data, output_dir):
    ...
    fig_dwn = px.bar(dwn_data, x='encoding', y='mean', color='dataset', error_y='std', barmode='group', title='DWN: Accuracy by Encoding and Dataset')
    fig_dwn.write_html(os.path.join(output_dir, 'dwn_accuracy.html'))

    # Graph: Accuracy by Encoding and Dataset for Wisard
    fig_wisard = px.bar(wisard_data, x='encoding', y='mean', color='dataset', error_y='std', barmode='group', title='Wisard: Accuracy by Encoding and Dataset')
...
def create_time_comparison_graph(dwn_delta_time, wisard_delta_time, output_dir):
    ...
    fig_time = px.bar(combined_delta_time, x='encoding', y='mean', color='dataset', error_y='std', barmode='group', 
                      facet_col='model', title='Delta Time by Encoding and Dataset')
...
def create_scatter_config_graph(dwn_scatter_config, wisard_scatter_config, output_dir):
    ...
    fig_scatter = px.scatter(combined_scatter_config, x='num_slices', y='num_dimensions', color='dataset', 
                             size='mean', facet_col='model', title='Scatter Code Configurations: num_slices vs num_dimensions')
    fig_scatter.write_html(os.path.join(output_dir, 'scatter_code_config.html'))

# Write conclusions to Markdown file
def write_conclusions_to_md(dwn_best_encoding, wisard_best_encoding, dwn_scatter_config, wisard_scatter_config, dwn_delta_time, wisard_delta_time, output_file):
    with open(output_file, 'w') as f:
        ...
        f.write('### DWN\n\n')
        f.write(dwn_best_encoding.to_markdown(index=False))
        f.write('\n\n### Wisard\n\n')
        f.write(wisard_best_encoding.to_markdown(index=False))
        f.write('\n\n')
...
def main():
    ...
    create_scatter_config_graph(dwn_scatter_config, wisard_scatter_config, 'output/graphs')
